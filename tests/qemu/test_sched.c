 ...
 *
 * mm_halow_sched.c cannot be tested on the host: its core is a naked function that
...
static void test_task_runs_and_is_reaped(void) {
    ...
    ran_count = 0;
    mm_halow_task_t *t = mm_halow_sched_task_create(task_runs_once, NULL, 256, "once");
...
static void test_core_registers_survive(void) {
    ...
    mm_halow_sched_deinit();
    reg_result = -1;
    mm_halow_task_t *t = mm_halow_sched_task_create(task_core_regs, NULL, 256, "regs");
    check(t != NULL, "task_create returned NULL");
    mm_halow_sched_run();   // runs up to the yield
    mm_halow_sched_run();   // resumes and finishes
...
// mm_halow_context_switch(). If that vpush/vpop pair is wrong -- or if the FPU was
...
static void test_fpu_registers_survive(void) {
    ...
    mm_halow_sched_deinit();
    fpu_result = -1;
    mm_halow_task_t *t = mm_halow_sched_task_create(task_fpu_regs, NULL, 512, "fpu");
    check(t != NULL, "task_create returned NULL");
    mm_halow_sched_run();
    mm_halow_sched_run();
...
static void test_tasks_run_in_creation_order(void) {
    ...
    for (uintptr_t i = 1; i <= 3; i++) {
        check(mm_halow_sched_task_create(task_order_a, (void *)i, 256, "ord") != NULL,
...
static void test_argument_is_passed(void) {
    ...
    mm_halow_sched_deinit();
    arg_seen = 0;
    mm_halow_sched_task_create(task_arg, (void *)0xDEADBEEF, 256, "arg");
    mm_halow_sched_run();
...
static void test_wait_returns_true_when_satisfied(void) {
    mm_halow_sched_deinit();
    cond_calls = 0;
    check(mm_halow_sched_wait(cond_true, NULL, 100), "wait should succeed immediately");
...
static void test_wait_times_out(void) {
    ...
    uint32_t before = mm_halow_test_event_waits;
    check(!mm_halow_sched_wait(cond_false, NULL, 5), "wait should time out and return false");
    check(mm_halow_test_event_waits > before, "a wait that times out should have slept, not spun");
}

static void test_wait_zero_timeout_polls_once(void) {
    mm_halow_sched_deinit();
    cond_calls = 0;
    check(!mm_halow_sched_wait(cond_false, NULL, 0), "zero timeout should return false");
...
static void task_reentrant(void *arg) {
    ...
    reentrant_ran++;
    reentrant_depth++;
    mm_halow_sched_run();   // must not recurse into this task again
    reentrant_depth--;
}
...
static void test_run_is_not_reentrant(void) {
    ...
    reentrant_ran = 0;
    reentrant_depth = 0;
    mm_halow_sched_task_create(task_reentrant, NULL, 512, "reent");
    mm_halow_sched_run();
...
static mm_halow_task_t *stack_task;
...
static void task_deep_stack_user(void *arg) {
    ...
    (void)sum;
    stack_free_seen = mm_halow_sched_stack_free_words(stack_task);
}
...
static void test_stack_watermark(void) {
    ...
    stack_free_seen = 0;
    stack_task = mm_halow_sched_task_create(task_deep_stack_user, NULL, words, "wm");
    check(stack_task != NULL, "task_create returned NULL");

    // Before it runs, exactly the initial context frame is unpainted.
    stack_free_before = mm_halow_sched_stack_free_words(stack_task);
    ...
    // starts with sp at the very top, so the first MM_HALOW_CONTEXT_WORDS it uses
...
static void test_deinit_clears_state(void) {
    pool_reset();
    mm_halow_sched_deinit();
    mm_halow_sched_task_create(task_runs_once, NULL, 256, "d");
    mm_halow_sched_deinit();
    check(mm_halow_sched_task_current() == NULL, "current task not cleared by deinit");
    ran_count = 0;
    mm_halow_sched_run();
...
static void test_task_capacity(void) {
    ...
    int created = 0;
    for (int i = 0; i < 16; i++) {
        if (mm_halow_sched_task_create(task_runs_once, NULL, 128, "cap") != NULL) {
            created++;
        }
    }
    check(created > 0, "no tasks could be created at all");
    check(created <= 8, "task table grew past its bound");
    check(mm_halow_sched_task_create(task_runs_once, NULL, 128, "over") == NULL,
...
static void test_claim_is_exclusive(void) {
    pool_reset();
    mm_halow_sched_deinit();
    check(mm_halow_sched_claim(), "first claim was refused");
    mm_halow_sched_release();
    check(mm_halow_sched_claim(), "claim was not released");
    mm_halow_sched_release();
}
...
static void test_claim_nests_within_a_context(void) {
    pool_reset();
    mm_halow_sched_deinit();
    check(mm_halow_sched_claim(), "outer claim was refused");
    check(mm_halow_sched_claim(), "a nested claim from the same context was refused");
    mm_halow_sched_release();
    check(mm_halow_sched_claim(), "the claim was dropped by the inner release");
    mm_halow_sched_release();
    mm_halow_sched_release();
    check(mm_halow_sched_claim(), "the claim was not released by the outermost release");
    mm_halow_sched_release();
}

static void test_deinit_releases_the_claim(void) {
    pool_reset();
    check(mm_halow_sched_claim(), "claim was refused before deinit");
    mm_halow_sched_deinit();
    check(mm_halow_sched_claim(), "deinit left the claim held");
    mm_halow_sched_release();
}
...
static void test_runaway_task_cannot_starve_the_system(void) {
    ...
    mm_halow_test_ticks = 0;
    spinner_laps = 0;
    mm_halow_sched_task_create(task_spins_forever, NULL, 512, "spin");
...
// The pass loop runs between task switches, where mm_halow_task_cur is NULL.  The
...
static void task_burns_a_whole_turn(void *arg) {
    ...
    for (;;) {
        slow_task_turns++;
        mm_halow_test_ticks += 100;     // one turn, far past the budget
        mm_halow_sched_yield();
    }
}
...
static void test_pass_loop_stops_when_out_of_time(void) {
    ...
    mm_halow_test_ticks = 0;
    slow_task_turns = 0;
    mm_halow_sched_task_create(task_burns_a_whole_turn, NULL, 512, "slow");
...
static bool cond_false_until_released(void *arg) {
    (void)arg;
    cond_calls++;
    return mm_halow_test_ticks >= (uint32_t)forever_release_after;
}
...
static void test_wait_forever_outlives_the_cap(void) {
    ...
    check(mm_halow_sched_wait(cond_false_until_released, NULL, 0xFFFFFFFFu),
        "an infinite wait gave up instead of waiting");
    check(mm_halow_test_ticks >= 60000, "the wait was satisfied early");
}
...
static void test_wait_forever_is_capped_in_teardown(void) {
    ...
    check(!mm_halow_sched_wait(cond_false, NULL, 0xFFFFFFFFu),
...
static void test_budget_is_not_spent_when_idle(void) {
    ...
    mm_halow_test_ticks = 0;
    mm_halow_sched_run();
    check(!mm_halow_sched_over_budget(), "a fresh pass started out of time");
